...
pub type LineHeightScale = Rc<dyn Fn(&Range<usize>, &Rope, u64) -> f32>;
        ...
        F: FnMut(&str, Pixels) -> Vec<gpui::Boundary>,
    {
        ...
        for row in plan.lo..=plan.hi {
            ...
            let height_scale = match &self.height_scale {
                Some(scale) => normalize_scale(scale(&line_range, changed_text, self.generation)),
                None => 1.0,
            };
...
mod tests {
    ...
    fn wrapper_with_scales_from(source: &Rc<std::cell::RefCell<Rope>>) -> TextWrapper {
        ...
        wrapper.height_scale = Some(Rc::new(move |range: &Range<usize>, _: &Rope, _: u64| {
